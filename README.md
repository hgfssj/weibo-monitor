...
- **盘中实时**（A股资金面页顶部）：`serve.py` 的 `/api/intraday` 实时代理腾讯指数分时（7 指数当日分钟线，中证2000无分时源）、申万行业指数分时（ak.index_min_sw，一级行业 31 个全覆盖 + 二级热门 10 个，与日线图同口径，低并发含重试防限流）与新浪行业实时成交（49 个行业成交额/占比），服务端 30 秒缓存，前端 20 秒轮询；仅交易时段数据会变化，收盘后展示当日全天分时。
