...
- **盘中实时**（页顶部）：主要指数当日分时（腾讯，7 指数）+ 申万行业指数分时（一级 31 全覆盖 + 二级热门 10，与日线图同口径）+ 新浪 49 行业实时成交额占比表（占比条含前交易日橙线对比）；服务端 30 秒缓存 + 前端 20 秒轮询
- **主要指数走势**：近3月/半年/一年可选，8 个指数（上证/上证50/沪深300/中证500/中证1000/中证2000/创业板指/科创50）期初归一对比
...
- **上证指数**：自动从 A股资金面宏观数据里取上证指数收盘填充；统计源 T+1 缺口在收盘后用腾讯分时末点自动补当日值（期货页与宏观页同步生效）
...
- **盘中实时**（A股资金面页顶部）：`serve.py` 的 `/api/intraday` 实时代理腾讯指数分时（7 指数当日分钟线，中证2000无分时源）、申万行业指数分时（ak.index_min_sw，一级行业 31 个全覆盖 + 二级热门 10 个，与日线图同口径，低并发含重试防限流）与新浪行业实时成交（49 个行业成交额/占比），服务端 30 秒缓存，前端 20 秒轮询；仅交易时段数据会变化，收盘后展示当日全天分时。行业成交占比表支持与前交易日对比：服务端每日留存收盘快照（`data/intraday_daily_snapshot.json`），次交易日起占比条上以橙色竖线标出前一日占比（同口径新浪实时，东财历史接口在本环境不可用）。
