...
- **盘中实时**（A股资金面页顶部）：`serve.py` 的 `/api/intraday` 实时代理腾讯指数分时（7 指数当日分钟线，中证2000无分时源）、申万行业指数分时（ak.index_min_sw，15 个重点行业与日线图同口径）与新浪行业实时成交（49 个行业成交额/占比），服务端 15 秒缓存，前端 20 秒轮询；仅交易时段数据会变化，收盘后展示当日全天分时。
